...
- **`wai-learn.ts`** — Backs `/wai-learn`: records/verifies project facts (learned.json, capped at 200). Facts may carry `kind: "decision"`; decisions are injected into review prompts as a "Known decisions — do NOT re-flag" block and into the main agent's injected context with a `[decision]` marker. `findLearnedFacts` supports kind filtering.
