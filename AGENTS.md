...
- **`actions/range.ts`** — Shared diff-range selection for the whole-tree actions (`src/actions/range.ts`): `resolveRangeBase` picks the diff base per policy (`incremental` for review/test/security: pending anchor wins over the accepted baseline; `holistic` for judge: baseline first) with validated anchors, clean/dirty fallbacks (HEAD~1, empty tree for root commits), and base absolutization; `updateRangeState`/`pinAttemptedRange` implement the pass-only baseline advancement, pending-anchor pinning, and the scoped/inconclusive/coverage guards. Review/test/security/judge all use it; only review mutates the range state.
