...
export default async function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "A 'code change' = a cohesive edit batch / one plan step's worth of edits — review once per batch before moving on (not after every keystroke); fixes made after feedback are a new batch and need a re-review of the feedback-fix plus directly affected files (don't re-review unchanged prior batches — a passing review closes that batch).",
      "Scope reviews to the edited files plus directly affected tests/types/config files — not the whole repo. Use review:true with files:[...] to limit the diff, or exclude:[...] to skip generated output.",
      ...
      "A review that errored (model unavailable, budget skip, fail-closed guidance) is blocking just like a non-pass verdict: it blocks advancing to the next batch, judge, commit, or done. Resolve the cause and re-run; if the error is unresolvable, report blocked instead of looping on retries.",
      ...
      "Workflow order: plan → implement → review (per batch, after checks) → fix-and-re-review until pass → judge on the complete change → then done:true to sync the plan tracker. Committing is the USER's optional step — never auto-commit; the user reviews the diff and commits themselves.",
  ...
  pi.registerTool({
    ...
    description:
      ...
      "The path may be project-relative or absolute (e.g. a file in Downloads). PDFs use their text layer when " +
      "present (any text model works) and are rendered to images when scanned. Image analysis requires the sdk " +
